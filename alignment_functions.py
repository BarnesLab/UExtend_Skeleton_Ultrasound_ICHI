...
def OPA(A_t, B_t, reflect=False):
    ...
    if reflect:
        ...
        R = u @ v_t
        return np.transpose(R @ A)  # Return landmarks x dim
    else:
...
def cov_int(beta_dot_c_t, delta_t, c):
    ...
    for t in range(beta_t.shape[2] - 1):
        beta_t[:, :, t + 1] = exp(beta_t[:, :, t], parallel(beta_dot_c_t[:, :, t], c, beta_t[:, :, t]) * delta_t)
...
def srvf(beta_dot_t, delta_t):
    ...
    norm = np.sqrt(np.linalg.norm(beta_dot_t, axis=(0, 1)))
    ...
    # print(norm)
    for i in range(beta_dot_t.shape[2]):
        if norm[i] > thresh:
...
def tsrvf(beta_t, delta_t, c):
    ...
    beta_dot_c = parallel_vf(beta_dot, beta_t, c)

    q_t = srvf(beta_dot_c, delta_t)
...
def rotate_trajectory_align(mu, traj, reflect=False):
    """Aligns beta with mu"""
    traj_aligned = np.zeros(shape=traj.shape)

    for tt in range(traj.shape[2]):
        # print(tt, traj_aligned.shape)
        traj_aligned[:, :, tt] = OPA(traj[:, :, tt], mu[:, :, tt], reflect=reflect)
...
def temporal_align(mu, beta, delta_t):
    ...
    gamma_inv = fdasrsf.curve_functions.optimum_reparam_curve(q_mu_flat, q_beta_flat, method="DP")
...
def temporal_rotation_align(mu, beta, t, iterations=10, tol=10 ** (-5), reflect=False):
    ...
    for iteration in tqdm(range(iterations)):
        error = np.linalg.norm(mu - beta_hat)
        history.append(error)

        beta_hat = rotate_trajectory_align(mu, beta_hat, reflect=reflect)
...
def frechet(betas, t, mu_init, iterations=50, plot=True, tol=10 ** (-5)):
    ...
    for iteration in tqdm(range(iterations)):
        ...
        for tt in range(mu.shape[2]):
            for n in range(N):
                tangent_vec[:, :, tt, n] = log(mu[:, :, tt], betas_aligned[n][:, :, tt])  # make sure to shoot a vector to the aligned betas
                mean_tangent_vec[:, :, tt] += tangent_vec[:, :, tt, n] / float(N)

            mu[:, :, tt] = exp(mu[:, :, tt], epsilon * mean_tangent_vec[:, :, tt])

        error = np.linalg.norm(mean_tangent_vec) ** 2
        history.append(error)
...
def plot_triangle_time(fig, beta, ax):
    ...
    ani = FuncAnimation(fig, update, frames=beta.shape[2], blit=False, repeat=True, interval=100)
    return ani
